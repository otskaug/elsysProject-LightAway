...
void loop() {
  ...
  for(angle = 0; angle < 181; angle+=2){ // One "Swipe". Had to increase the angle by two at a time due to problems with memory storage space for dynamic variables
    ...
    if (tfmini.readData()) {  // read the data frame sent by the mini, if valid dataframe do calculations
    
      dist = tfmini.getDistance();  // Distance "default in CM"
      ...
      servo.write(180); // make sure the servo is at 180 degrees before one swipe ends 
      }
    } // if tfmini statement 
